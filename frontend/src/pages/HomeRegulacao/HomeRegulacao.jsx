...
function ModalDevolverMedico({ isOpen, onClose, selectedPaciente, formValue }) {
  ...
  const handleDevolver = async () => {
    ...
    try {
      const response = await axios.patch(`http://18.230.66.70:8000/prescricoes/${selectedPaciente.id}/devolver_regulacao/`, {
...
function ModalAgendamento({ isOpen, onClose, selectedPaciente, selectedLeito, formValue }) {
  ...
  const handleAgendar = async () => {
    ...
    try {
      const response = await axios.patch(`http://18.230.66.70:8000/prescricoes/${selectedPaciente.id}/agendar_paciente/${selectedLeito.id}/`, {
        id_usuario: localStorage.getItem('idUser'),
        mensagem: formValue.mensagem  // Incluir a mensagem no corpo da requisição
      });
...
function ModalInternacao({ isOpen, onClose, selectedPaciente }) {
  ...
  const handleInternar = async () => {
    ...
    try {
      const response = await axios.patch(`http://18.230.66.70:8000/prescricoes/${selectedPaciente.id}/internar/`, {
        id_usuario: localStorage.getItem('idUser'),
      });
      console.log("Prescrição devolvida com sucesso:", response.data);
...
function ModalTransferencia({ isOpen, onClose, selectedPaciente, formValue }) {
  const handleTransferir = async () => {
    ...
    try {
      const response = await axios.patch(`http://18.230.66.70:8000/prescricoes/${selectedPaciente.id}/transferir/`, {
        id_usuario: localStorage.getItem('idUser'),
...
function QuadroFicha({ selectedPaciente, historico }) {
  ...
  useEffect(() => {
    const fetchLeitos = async () => {
      try {
        const response = await axios.get('http://18.230.66.70:8000/leitos/lista/');
...
function HomeRegulacao() {
  ...
  useEffect(() => {
    const fetchPacientes = async () => {
      try {
        const response = await axios.get('http://18.230.66.70:8000/pacientes/lista_regulacao/');
